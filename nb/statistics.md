...
## Data
Data that we collect is often be called realizations that are generated by a random function (i.e. a distribution). Data can come in several types and the type determines the test we can use.

### Continuous
This is the most common data type in biology and is the most common depedent variable type that we collect. Continuous means we could collect any value with infinite precision however, in reality, we typically get real values. This is rheobase, frequency, membrane resistance, etc. Continuous data can come from a variety of [distributions](=distributions).

### Categorical
Categorical data is very common in biology and is most commonly an indepedent variable. Categorical data has a finite number of values that cannot be order and usually are the result of some grouping like genotype or sex. Categorical data can be depedent a variable (think the y in y=mx+b).

### Ordinal
Ordinal data is probably less common in biology. It is data with a finite number of values (discrete) that have some intrisic order like more votes better than less or dates. Something that is inherently ordered but not continuous.
...
### Parametric
We covered this in the [distributions chapter](=distributions) but parametric just means you have distributions that have parameters to describe them such as mean and STD. Parametric statistics are most of what we will cover.
### Non-parametric
These statistical tests are often rank-based tests. They essentially rank two groups and compare how many values in one group are larger than those in the other group. Non-parametric statistics do not assume your data follows a known distribution but some do assume that each group follows the same distribution.
