# Ion channels
This chapter will cover some of the major ion channels and how they effect neural function. Each section is not a comprehensive coverage of the channels but act more as a starting point. I will not cover structure of the channels unless is pertains to how it related to the function of the cell. We cover ion channels because they are the primary way which charge or current can go across the membrane. The membrane is practically impermeable to ions thus the neurons need a way for charge to distribute itself across the membrane. Ion channels are also resonsible for setting the resistance of a neuron. Simplistically, the number of opened and closed channels sets the membrane resistance. The distribution of types of ion channels and their functional properites in turn sets the functional properties of the neuron.
...
### SK channels
SK channels as small conductance calcium-activated potassium channels. SK channels can limit excitatory postsynaptic potentials, regulate Ca++ signaling, limit intrinsic excitability and affect pacemaking activity [@foisStructureFunctionPharmacology2026]. SK channels have been found to be regulated by L-type, T-type and N-type calcium channels[@wolfartSelectiveCouplingTType2002a; @DifferentCalciumChannels1995, @belghaziHighResolutionProteomicsUnravel]. There is likely cell-type dependent differences in how SK channels are regulated by calcium influx. The SK channels expressed in the brain are Kcnn1 (SK1), Kcnn2 (SK2), and Kcnn3 (SK3). Kcnn4 is not expressed in the brain. The primary blocker of SK channels is apamin, a peptide toxin [@foisStructureFunctionPharmacology2026]. Apamin blocks SK1 and SK3 with lower potency. There are a host of other synthetic SK channel blockers with varying potency and selectivity [@foisStructureFunctionPharmacology2026].
...
### A current potassium channels
A current potassium channels activate at subthreshold voltages and rapidly inactivate[@caiMultipleModesAType]. They typically activate during the AHP thus regulate the interspike interval.
```{image} ../data/gene_figures/atype_k.png
:alt: A current potassium channel gene expression
:align: center
```
